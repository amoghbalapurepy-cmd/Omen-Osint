...
async function tavilySearch(q, apiKey, maxResults=20) {
  return postJson('https://api.tavily.com/search', {
    query: q,
    topic: 'general',
    search_depth: 'basic',
  ...
  }, 20000, {'Authorization': `Bearer ${apiKey}`});
}

async function runWebSearch(input, res) {
  const apiKey = process.env.TAVILY_API_KEY;
  if (!apiKey) {
    writeEvent(res, {type:'search_error', error:'TAVILY_API_KEY is not set. Set it in the same terminal before starting server.js.'});
    writeEvent(res, {type:'done'});
    return;
  }
  const queries = makeWebSearchQueries(input);
  const seen = new Set();
  writeEvent(res, {type:'search_start', input, provider:'Tavily', query_count:queries.length});
  const tasks = queries.map(async (q, index) => {
    writeEvent(res, {type:'query', index:index+1, total:queries.length, q});
    const r = await tavilySearch(q, apiKey, 20);
    if (r.error) { writeEvent(res, {type:'search_error', query:q, error:r.error}); return; }
    if (!r.ok) {
      const detail = r.data?.detail || r.data?.error || r.data?.message || `Tavily returned HTTP ${r.status}`;
      writeEvent(res, {type:'search_error', query:q, http:r.status, error:detail}); return;
    }
    ...
    for (const item of results) {
      ...
      seen.add(url); emitted++;
      writeEvent(res, {type:'result',query:q,title:String(item?.title || 'Untitled result'),url,description:String(item?.content || ''),score:typeof item?.score === 'number' ? item.score : null,publishedAt:item?.published_date || null});
    }
    writeEvent(res, {type:'query_done', query:q, returned:results.length, emitted});
  });
  await Promise.allSettled(tasks);
  writeEvent(res, {type:'done', count:seen.size});
}
...
async function checkProvider(spec, res) {
  ...
  if (r.error) { writeEvent(res, {type:'result', provider:spec.name, status:'unverified', reason:r.error, profile:spec.profile}); return; }
  if (spec.miss && spec.miss(r)) { writeEvent(res, {type:'result', provider:spec.name, status:'miss', http:r.status, profile:spec.profile}); return; }
  if (!r.ok) { writeEvent(res, {type:'result', provider:spec.name, status:'unverified', http:r.status, reason:'provider returned a non-success response; not treated as a miss', profile:spec.profile}); return; }
  try {
  ...
  } catch(e) { writeEvent(res, {type:'result', provider:spec.name, status:'unverified', http:r.status, reason:'response format could not be verified', profile:spec.profile}); }
}
...
const server=http.createServer(async (req,res)=>{
  ...
  if(req.method==='GET' && u.pathname==='/api/websearch'){
    ...
    if(!q || q.length>400) return send(res,400,JSON.stringify({error:'Search query must be 1-400 characters.'}));
    ...
    await runWebSearch(q,res); res.end(); return;
  }
  if(req.method==='GET' && u.pathname==='/api/recon'){
    ...
    await runRecon(username,res); res.end(); return;
  }
  ...
  if(req.method==='GET' && (u.pathname==='/' || u.pathname==='/index.html')){
    fs.readFile(INDEX,'utf8',(err,html)=>{
      if(err) return send(res,500,'OMEN index unavailable','text/plain');
      const enhanced=html.replace('</head>','<link rel="stylesheet" href="/omen-ui.css"><script src="/omen-ui.js" defer></script></head>');
      send(res,200,enhanced,'text/html; charset=utf-8');
    });
    return;
  }
