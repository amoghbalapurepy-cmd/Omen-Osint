...
async function runWebSearch(input, res) {
  ...
  // Run query variants concurrently. Every emitted result comes directly from Tavily.
  const tasks = queries.map(async (q, index) => {
    ...
    if (r.error) {
      writeEvent(res, {type:'search_error', query:q, error:r.error});
      return;
    }
    if (!r.ok) {
      const detail = r.data?.detail || r.data?.error || r.data?.message || `Tavily returned HTTP ${r.status}`;
      writeEvent(res, {type:'search_error', query:q, http:r.status, error:detail});
      return;
    }
